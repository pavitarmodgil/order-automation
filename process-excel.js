// First install xlsx: npm install xlsx
// Then run: node process-excel.js

const XLSX = require('xlsx');
const fs = require('fs');

console.log('📊 Reading hnm.xlsx file to extract stock items...\n');
...
try {
    ...
    data.forEach((row, index) => {
        ...
        const name = row['Name'] || row['NAME'] || row['Item Name'] || row['Stock Item'] || row['Product'] || row['Description'] || row['Item'];
        const rate = row['Rate'] || row['RATE'] || row['Price'] || row['Cost'] || row['Amount'] || row['Value'];
        const unit = row['Unit'] || row['UNIT'] || row['UOM'] || row['Unit of Measure'] || row['Units'];
    ...
    console.log('\n💡 Please check:');
    console.log('   1. Excel file exists at: backend/data/hnm.xlsx');
